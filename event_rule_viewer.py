...
class GraphView(QWidget):
    ...
    def get_displayed_time(self, time):
        ...
        In normalized mode, this converts from true time to normalized time.
        In true timing mode, this simply returns the original time.
        
        Args:
            time: The original time value to convert
            
        Returns:
            The time value to use for display calculations.
        """
        if not self.normalized_timing:
            return time
            
        # In normalized mode, convert from original to normalized time
        ...
        if not self.original_times:
            return time  # No normalization data available
        ...
        if times_before and times_after:
            ...
            # After the last transition, extrapolate using 500ps spacing
            t_before = max(times_before)
            last_normalized = self.time_mapping[t_before]
            
            # Calculate how many 500ps intervals past the last known time
            time_diff = time - t_before
            extra_intervals = time_diff / 500
            return last_normalized + extra_intervals * 500
            
        elif times_after:
            # Before the first transition, extrapolate using 500ps spacing
            t_after = min(times_after)
            first_normalized = self.time_mapping[t_after]
            
            # Calculate how many 500ps intervals before the first known time
            time_diff = t_after - time
            extra_intervals = time_diff / 500
            return first_normalized - extra_intervals * 500
            
        # Fallback - shouldn't reach here if data is available
        return time
    ...
    def time_to_x(self, time):
        ...
        In normalized mode, this uses the normalized time.
        In true timing mode, this uses the original time.
        """
    ...
    def x_to_time(self, x):
        ...
        In normalized mode, this returns a normalized time.
        In true timing mode, this returns an original time.
        """
        # Calculate raw time from screen coordinates
        time = (x - self.offset_x) / self.scale
        
        # In normalized mode, we should try to snap to multiples of 500ps
        # since that's our normalization interval
        if self.normalized_timing and self.normalized_times:
            # Find the nearest normalized time
            nearest_normalized = min(self.normalized_times, 
                                    key=lambda t: abs(t - time))
            
            # If we're very close to a normalized time, snap to it
            if abs(nearest_normalized - time) < 250:  # Half the normalization interval
                return nearest_normalized
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            if self.normalized_timing:
                # Since normalized times use 500ps spacing, use multiples of 500ps for intervals
                ...
                # Ensure our interval is a multiple of the normalization spacing (500ps)
                base_interval = max(500, base_interval)
                if base_interval % 500 != 0:
                    base_interval = (base_interval // 500 + 1) * 500
            else:
            ...
            while interval < time_range and (interval * self.scale) < min_pixels_per_interval:
                interval *= 2
                # For normalized mode, make sure we keep multiples of 500
                if self.normalized_timing and interval % 500 != 0:
                    interval = (interval // 500 + 1) * 500
                
            # Calculate first time marker
            if self.normalized_timing:
                # For normalized timing, ensure we're starting at a multiple of the interval
                # that is at or just before the visible_min_time
                first_time = (int(visible_min_time / interval)) * interval
                # Ensure it's a multiple of 500ps
                if first_time % 500 != 0:
                    first_time = (first_time // 500) * 500
            else:
                first_time = (int(visible_min_time / interval)) * interval
